...
public class ImagePreviewPanel extends JPanel implements PropertyChangeListener {
	...
	private static final long serialVersionUID = 7572722697206754547L;
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		if (propertyName.equals(JFileChooser.SELECTED_FILE_CHANGED_PROPERTY)) {
			 ...
			 */
			if ((name != null) && name.toLowerCase().endsWith(ImageFilter.GIF)
					|| name.toLowerCase().endsWith(ImageFilter.JPEG)
					|| name.toLowerCase().endsWith(ImageFilter.JPG)
					|| name.toLowerCase().endsWith(ImageFilter.PNG)
					|| name.toLowerCase().endsWith(ImageFilter.TIF)
					|| name.toLowerCase().endsWith(ImageFilter.TIFF)) {

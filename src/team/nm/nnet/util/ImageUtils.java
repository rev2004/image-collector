...
public class ImageUtils {
	 ...
	 *            Anh can resze
	 * @param width
	 *            Chieu rong can resize
	 * @param height
	 *            Chieu dai can resize
	...
	public static BufferedImage flip(BufferedImage bufferedImage,
		...
		if (vertical) {
			flipedBuff = new BufferedImage(width, height,
					bufferedImage.getType());
			Graphics2D g = flipedBuff.createGraphics();
			g.drawImage(bufferedImage, 0, 0, width, height, 0, height, width,
					0, null);
			g.dispose();
		} else {
			flipedBuff = new BufferedImage(width, height,
					bufferedImage.getType());
			Graphics2D g = flipedBuff.createGraphics();
			g.drawImage(bufferedImage, 0, 0, width, height, width, 0, 0,
					height, null);
			g.dispose();
		}
		return flipedBuff;
	 ...
	 *            Anh can xoay
	 * @param angle
	 *            Goc xoay (Tinh bang do)
	 * @return Ket qua xoay
	 */
	public static BufferedImage rotate(BufferedImage bufferedImage, double angle) {
		int width = bufferedImage.getWidth(null);
		int height = bufferedImage.getHeight(null);
		BufferedImage rotatedBuff = new BufferedImage(width, height,
				bufferedImage.getType());
	...
	public static BufferedImage rotateAndScale(BufferedImage bufferedImage,
			double angle) {
		...
		at.rotate(Math.toRadians(angle), originWidth / 2.0, originHeight / 2.0);
